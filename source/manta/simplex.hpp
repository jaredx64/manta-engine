...
public:
	Simplex() { };
	Simplex( u64 seed ) { this->seed( seed ); }

	void seed( u64 seed );

	float sample_fast( float x, float y ) const;
	float sample_unorm_fast( float x, float y ) const;
	float sample_fbm_fast( float x, float y, float f, float a, float l, float p, int o ) const;
