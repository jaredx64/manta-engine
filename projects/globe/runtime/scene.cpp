...
{
	if( target.width != Window::width_pixels() || target.height != Window::height_pixels() )
	{
		target.resize( Window::width_pixels(), Window::height_pixels() );
	}
}
...
void scene_init()
{
	...
	Scene::rtSceneMSAA.init( Window::width_pixels(), Window::height_pixels(), rtSceneMSAADescription );
	...
	Scene::rtSceneComposite.init( Window::width_pixels(), Window::height_pixels(), rtSceneMSAADescription );
}
...
void scene_draw_3d( const Delta delta )
{
	...
	passMSAA.target( 0, Scene::rtSceneMSAA );

	GfxRenderPass passComposite;
	validate_render_target( Scene::rtSceneComposite );
	passComposite.target( 0, Scene::rtSceneComposite );
	...
	Gfx::render_pass_begin( passComposite );
	{
		...
		cmd.shader( Shader::sh_composite_quad );
...
{
	Gfx::set_matrix_mvp_2d_orthographic( 0.0, 0.0, 1.0, 0.0,
		Window::width_logical(), Window::height_logical() );
	Gfx::sampler_filtering_mode( GfxSamplerFilteringMode_NEAREST );
	...
	draw_text( fnt_iosevka, size, Window::width_logical() - 8.0f - lineWH.x, 8.0f + 20.0 * 0, c_ltgray, line );

	line = "Zoom: Q, E";
	lineWH = text_dimensions( fnt_iosevka, size, line );
	draw_text( fnt_iosevka, size, Window::width_logical() - 8.0f - lineWH.x, 8.0f + 20.0 * 1, c_ltgray, line );

	line = "Orbit Mode: SPACE";
	lineWH = text_dimensions( fnt_iosevka, size, line );
	draw_text( fnt_iosevka, size, Window::width_logical() - 8.0f - lineWH.x, 8.0f + 20.0 * 2, c_ltgray, line );

	line = "Toggle Clouds: C";
	lineWH = text_dimensions( fnt_iosevka, size, line );
	draw_text( fnt_iosevka, size, Window::width_logical() - 8.0f - lineWH.x, 8.0f + 20.0 * 3, c_ltgray, line );

	line = "Show Labels: L";
	lineWH = text_dimensions( fnt_iosevka, size, line );
	draw_text( fnt_iosevka, size, Window::width_logical() - 8.0f - lineWH.x, 8.0f + 20.0 * 4, c_ltgray, line );

	line = "Show Debug: F1";
	lineWH = text_dimensions( fnt_iosevka, size, line );
	draw_text( fnt_iosevka, size, Window::width_logical() - 8.0f - lineWH.x, 8.0f + 20.0 * 5, c_ltgray, line );

	line = "Jump To: Left Mouse";
	lineWH = text_dimensions( fnt_iosevka, size, line );
	draw_text( fnt_iosevka, size, Window::width_logical() - 8.0f - lineWH.x, 8.0f + 20.0 * 6, c_ltgray, line );

	line = "Change Sun: Right Mouse";
	lineWH = text_dimensions( fnt_iosevka, size, line );
	draw_text( fnt_iosevka, size, Window::width_logical() - 8.0f - lineWH.x, 8.0f + 20.0 * 7, c_ltgray, line );
	...
	if( debug )
	{
		...
		draw_axis_3d( Window::width_logical() - 200, Window::height_logical() - 200,
			192, 192, View::forward, View::up );
	}
}
