...
#define ABSORPTION ( ( 1.0 / 48.0 ) * ( 128.0 / NUM_STEPS ) )

#define COLOR_ATMOSPHERE_DAY ( float4( 0.75, 0.75, 0.97, 1.00 ) )
#define COLOR_ATMOSPHERE_NIGHT ( float4( 0.05, 0.05, 0.1, 1.00 ) )
#define COLOR_AURORA_G ( float3( 0.00, 2.00, 0.00 ) )
#define COLOR_AURORA_R ( float3( 2.00, 0.75, 0.00 ) )
...
vertex_output VertexOutput
{
	...
	float3 start semantic( TEXCOORD );
};
...
float atmosphere_thickness( float3 point, float fraction )
{
	float elevation = clamp( length( point ) - R_EARTH, 0.0, R_ATMOSPHERE );
	float falloff = clamp( 1.0 - ( elevation / R_ATMOSPHERE ), 0.0, 1.0 );

	return pow( falloff, 3.0 ) * ABSORPTION * fraction;
}
...
float4 integrate_atmosphere( float4 color, float3 start, float3 forward,
	UniformsPipeline Pipeline, UniformsAtmosphere Uniforms )
{
	float3 colorAtmosphereDay = COLOR_ATMOSPHERE_DAY.xyz;//Uniforms.color.rgb;
	...
	for( int i = 0; i < NUM_STEPS; i++ )
	{
		...
		float absorption = atmosphere_thickness( sample, sampleFraction );
		...
		colorFinal += colorAtmosphere * absorption * ( 1.0 - nightness * 0.33 );
		alphaFinal = min( 1.0, alphaFinal + absorption * ( 1.0 - nightness * 0.33 ) );
		...
		if( auroraStrength > 0.0 )
		{
			...
			absorption *= pow( auroraStrength, 2.0 ) * ( 0.25 + ( 1.75 * nightness ) );
			colorFinal = lerp( colorFinal, colorAurora, absorption * ( 1.0 - alphaFinal ) );
			alphaFinal += absorption * ( 1.0 - alphaFinal );
		}
...
void fragment_main( FragmentInput In, FragmentOutput Out, UniformsPipeline Pipeline, UniformsAtmosphere Uniforms )
{
	...
	float4 color = integrate_atmosphere( Uniforms.color, In.start, direction, Pipeline, Uniforms );
